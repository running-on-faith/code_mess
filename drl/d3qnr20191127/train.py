...
def train_round_iter_func(round_n_per_target_day, target_avg_holding_days=[5, 7, 9]):
    """
    迭代器函数：每轮训练迭代参数 迭代器
    ...
    for round_n_sub in range(round_n_per_target_day):
        for days in target_avg_holding_days:
            ...
            agent_kwargs = dict(keep_last_action=math.pow(0.5, 1 / days), batch_size=512,
                                epsilon_memory_size=10, random_drop_best_cache_rate=0.05)
...
def _test_train_on_each_period():
    ...
    train_on_each_period(
        ...
        train_round_kwargs_iter_func=functools.partial(train_round_iter_func, round_n_per_target_day=2), n_step=120,
