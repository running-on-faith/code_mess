...
class DRLStg(StgBase):
    ...
    def __init__(self, instrument_type, unit=1):
        ...
        self.action_size = 3
        self.n_step = 60
        self.args = parse_args(['--nb_episodes', str(self.num_episodes)])
        ...
        if self.enable_load_model_if_exist:
            self.base_folder_path = folder_path = os.path.join(module_root_path, 'tf_saves_2019-07-11_10_43_40')
        else:
    ...
    def get_model(self, rebuild_model=False) -> DDQN:
        """
        建立模型前必须 get_factor 调用完毕，需要用到 self.input_size
        :param rebuild_model:
        :return:
        """
        if self._algorithm is None or rebuild_model:
            self._algorithm = DDQN(action_dim=self.action_size, state_dim=self.input_shape, args=self.args)
    ...
    def train(self, md_df, batch_factors, show_plot=False, has_try_n_times=None) -> pd.DataFrame:
        """
        ...
        """
        self._env = env = Account(md_df, data_factors=batch_factors, expand_dims=False)

        trade_date = str_2_date(md_df.index[-1])
        sess = get_session()
        set_session(sess)
        ...
        if has_try_n_times is None or has_try_n_times == 1:
            ...
            self.logger.debug("round=%d, env.A.total_value=%f",
                              has_try_n_times, env.A.total_value)
    ...
    def save_model(self, trade_date, key=None):
        ...
        file_path = self._algorithm.save_weights(path=file_path)
    ...
    def load_train_test(self, indexed_df, enable_load_model, rebuild_model=False, enable_train_if_load_not_suss=True,
                        enable_train_even_load_succ=False):
        if rebuild_model or self._algorithm is None:
    ...
    def predict_latest_action(self):
        ...
        """
        env = Account(self._md_df, data_factors=self._batch_factors, expand_dims=False)
        latest_state = env.latest_state()
        # TODO: 加入强化学习机制，目前只做action计算，没有进行状态更新
        action = self._algorithm.choose_action_deterministic(latest_state)
        return action
