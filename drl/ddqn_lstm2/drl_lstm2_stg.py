...
class DRL_LSTM_Stg(StgBase):
    ...
    def __init__(self, instrument_type, unit=1):
        ...
        self.action_size = 4  # close, long, short, keep 如果是3的话，则没有keep
        self.n_step = 60
    ...
    def get_model(self, rebuild_model=False) -> Agent:
        ...
        if self._agent is None or rebuild_model:
            ...
            self._agent = Agent(input_shape=self.input_shape, action_size=self.action_size,
    ...
    def get_factor(self, md_df):
        ...
        if self._data_factors_latest_date is None or self._data_factors_latest_date < trade_date_latest:
            ...
            self._batch_factors = data_arr_batch
    ...
    def get_date_file_path_pair_list(self):
        ...
        """
        # 获取全部文件名
        pattern = re.compile(r'weights_dqn_\d+.h5')
    ...
    def train(self, md_df, batch_factors, has_try_n_times=None, show_plot=False) -> pd.DataFrame:
        ...
        """
        trade_date = date_2_str(md_df.index[-1])
        agent = self._agent
        self._env = env = Account(md_df, data_factors=batch_factors, state_with_flag=True)
        ...
        for episode in range(self.num_episodes):
            ...
            while True:
                ...
                if (global_step >= agent.update_eval_batch_size and episode_step % self.train_step_size == 0) or done:
                    agent.update_eval()
                    ...
                    if done:
                        ...
                        if episode % self.show_log_pre_n_loop == 0 or episode == self.num_episodes - 1:
                            ...
                            if has_try_n_times is None or has_try_n_times == 1:
                                self.logger.debug("%s, episode=%d, data_observation.shape[0]=%d, env.A.total_value=%f",
                                                  trade_date, episode, env.A.data_observation.shape[0], env.A.total_value)
                            else:
                                self.logger.debug("%s, round=%d, episode=%d, env.A.total_value=%f",
                                                  trade_date, has_try_n_times, episode, env.A.total_value)
                        break

        # 样本内验证测试
        title = f'ddqn_lstm_round_{has_try_n_times}_episodes_{self.num_episodes}_{trade_date}'
        self.predict(show_plot=show_plot, title=title)

        reward_df = env.plot_data()
        path = f"weights_{has_try_n_times}_{self.num_episodes}.h5"
        folder_path = os.path.join(self.model_folder_path, date_2_str(trade_date))
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        file_path = os.path.join(folder_path, path)
        agent.save_model(path=file_path)
        logger.debug('model save to path: %s', file_path)
    ...
    def save_model(self, trade_date, key=None):
        ...
        file_path = os.path.join(
            folder_path,
            f"weights_dqn_{key}.h5" if key is not None else f"model_dqn_0.h5")
    ...
    def predict(self, show_plot=True, title=None):
        agent = self._agent
        env = Account(self._md_df, data_factors=self._batch_factors, state_with_flag=True)
        episodes_train = []
        ...
        if show_plot:
            ...
            plot_twin(value_df, self._md_df['close'], name=title)
